...
method List { self.child-nodes }
method Hash { self.attr        }
method Bool { True             }
method Str  { self.to-string   }
method gist { self.to-string   }

method all-text { self._all-text(1, @_) }
method ancestors { _select($_[0]->_collect($_[0]->_ancestors), $_[1]) }
method append { shift->_add(1, @_) }
method append_content { shift->_content(1, 0, @_) }
method at {
  my $self = shift;
...
method child_nodes { $_[0]->_collect(_nodes($_[0]->tree)) }

method children { _select($_[0]->_collect(_nodes($_[0]->tree, 1)), $_[1]) }
...
method descendant_nodes { $_[0]->_collect(_all(_nodes($_[0]->tree))) }

method find { $_[0]->_collect(@{$_[0]->_css->select($_[1])}) }

method following { _select($_[0]->_collect(@{$_[0]->_siblings(1)->[1]}), $_[1]) }
method following_nodes { $_[0]->_collect(@{$_[0]->_siblings->[1]}) }

method matches { shift->_css->matches(@_) }
...
method next      { $_[0]->_maybe($_[0]->_siblings(1, 0)->[1]) }
method next_node { $_[0]->_maybe($_[0]->_siblings(0, 0)->[1]) }
...
method parse { shift->_delegate(parse => @_) }

method preceding { _select($_[0]->_collect(@{$_[0]->_siblings(1)->[0]}), $_[1]) }
method preceding_nodes { $_[0]->_collect(@{$_[0]->_siblings->[0]}) }

method prepend { shift->_add(0, @_) }
method prepend_content { shift->_content(0, 0, @_) }

method previous      { $_[0]->_maybe($_[0]->_siblings(1, -1)->[0]) }
method previous_node { $_[0]->_maybe($_[0]->_siblings(0, -1)->[0]) }

method remove { shift->replace('') }
...
method tap { shift->Mojo::Base::tap(@_) }

method text { shift->_all_text(0, @_) }

method to_string { shift->_delegate('render') }

method tree { shift->_delegate(tree => @_) }

method type { shift->tree->[0] }
...
method wrap         { shift->_wrap(0, @_) }
method wrap_content { shift->_wrap(1, @_) }

method xml { shift->_delegate(xml => @_) }
...
method _build { shift->new->tree(shift)->xml(shift) }
...
method _css { Mojo::DOM::CSS->new(tree => shift->tree) }
...
method _maybe { $_[1] ? $_[0]->_build($_[1], $_[0]->xml) : undef }
...
method _parent { $_[0]->tree->[$_[0]->type eq 'tag' ? 3 : 2] }

method _parse { Mojo::DOM::HTML->new(xml => shift->xml)->parse(shift)->tree }
...
method _start { $_[0][0] eq 'root' ? 1 : 4 }
...
=head1 SYNOPSIS
  ...
  # Iterate
  $dom->find('p[id]')->reverse->each(method { say $_->{id} });
...
element as L<Mojo::DOM> objects.
  ...
  $dom->parse('<p><!-- Test --><b>123<!-- 456 --></b></p>')
    ->descendant_nodes->grep(method { $_->type eq 'comment' })
    ->map('remove')->first;

  # "<p><b>test</b>test</p>"
  $dom->parse('<p><b>123</b>456</p>')
    ->at('p')->descendant_nodes->grep(method { $_->type eq 'text' })
...
objects. All selectors from L<Mojo::DOM::CSS/"SELECTORS"> are supported.
  ...
  my $hash = $dom->find('*')->reduce(method { $a->{$b->tag}++; $a }, {});
